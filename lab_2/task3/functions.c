...
kState SearchSubstringsInFile(char* substring,char* path, Vector* lines, Vector* chars, Vector* fileIds, int fileId) {
    ...
    while (currentFile != EOF) {
        if (currentFile != *currentSubstr) {
            if (*currentSubstr == '\0') {
                code = PushVector(lines, currentLine);
                if (code != kS_OK) {
                    return code;
                }
                code = PushVector(chars, currentChar);
                if (code != kS_OK) {
                    return code;
                }

                code = PushVector(fileIds, fileId);
                if (code != kS_OK) {
                    return code;
                }
            }
...
kState FindEnterancesInFiles(Vector* lines, Vector* chars, Vector* fileIds, char* substring, int count, ...) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        code = SearchSubstringsInFile(substring, path, lines, chars, fileIds, i);
        if (code != kS_OK) {
            va_end(args);
            return code;
        }
